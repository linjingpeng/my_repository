			  ...
			  <div class="panel-body">
				<form id="userForm" role="form" >
				  ...
				  <div class="col-sm-4 col-sm-offset-3">
					...
					<input type="text" class="form-control" id="dictionary_value" placeholder="请输入字典数据名"><br>
				  </div>
				  <div class="col-sm-4 col-sm-offset-3">
					<label for="exampleInputPassword1">字典名</label>
					<input type="text" class="form-control" id="dictionary_name" placeholder="请输入字典名"><br>
				  </div>
				 
				  <div class="col-sm-4 col-sm-offset-3">
					<label for="exampleInputPassword1">字典类型编码</label>
					<input type="text" class="form-control" id="dictionary_type_code" placeholder="字典类型编码"><br>
				  </div>
				  <div class="col-sm-4 col-sm-offset-3">
					<label for="example1">字典类型名称</label>
					<input type="text" class="form-control" id="dictionary_type_name" placeholder="请输入字典类型名称"><br>
				  </div>
				  <div class="col-sm-4 col-sm-offset-3">
				  <button id="addBtn" type="button" class="btn btn-success"><i class="glyphicon glyphicon-plus"></i> 增加字典</button>
				  </div>
				</form>
			  </div>
			</div>
            ...
            $(function () {
			  ...
			  //返回
			    ...
			    $("#addBtn").click(function(){
			    	//alert($("input[name=dictionary_gender]:checked").val());
			    	var loadingIndex = null;
			    	$.ajax({
			    		type : "POST",
			    		url  : "${pageContext.request.contextPath }/dictionary/addDictionary.do",
			    		data : {
			    		...
			    		success : function(result) {
			    			...
			    			if ( result.success ) {
		                        layer.msg("字典增加成功", {time:1000, icon:6}, function(){
		                        	window.location.href = "${pageContext.request.contextPath }/dictionary/toDictionaryList.do";
		                        });
			    			} else {
		                        layer.msg("字典增加失败，请重新操作", {time:2000, icon:5, shift:6}, function(){

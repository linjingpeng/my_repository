			...
			<!-- 内容页 start-->
				<div class="panel panel-default">
					...
					<div class="panel-body">
								...
								<div class="input-group">
									<div class="input-group-addon">
										<select name="condition">
												<option value="admin_name">用户名</option>
												<option value="admin_account">账号</option>
										</select>
									</div>
						...
						<button id="btnDeleteChkAdmin" type="button" class="btn btn-danger"
							style="float: right; margin-left: 10px;">
							<i class=" glyphicon glyphicon-remove"></i> 删除
						</button>
						<button type="button" class="btn btn-primary"
							style="float: right;" onclick="window.location.href='${pageContext.request.contextPath}/admin/toAdminAdd.do'">
						...
						<div class="table-responsive">
							<table class="table  table-bordered">
								<thead>
									<tr>
										<th width="30"><input id="chkAllAdmin" type="checkbox"></th>
										<th>编号</th>
										<th>用户名</th>
										<th>账号</th>
										<th>状态</th>
								...
								<tbody>
									<c:forEach items="${requestScope.page.data }" var="admin" varStatus="status">
									<tr>
										<td><input name="adminId" value="${admin.admin_id }" type="checkbox"></td>
										<td>${status.count }</td>
										<td>${admin.admin_name }</td>
										<td>${admin.admin_account }</td>
										<td>${admin.status.dictionary_name }</td>
										<td>${admin.role.role_name }</td>
										<td>
											<button type="button" onclick="toAdminEdit(${admin.admin_id})" class="btn btn-primary btn-xs">
												<i class=" glyphicon glyphicon-pencil"></i>
											</button>
											<button type="button" onclick="deleteAdmin(${admin.admin_id},'${admin.admin_name}')" class="btn btn-danger btn-xs">
          ...
          //查询
		    $("#query").click(function(){
		    	...
		    	if ( queryText == "" ) {
		    		...
		    		window.location.href = "${pageContext.request.contextPath }/admin/toAdminList.do?"+condition+"="+queryText;
		    	}
		    	
		    })
            function toAdminEdit(adminId){
            	window.location.href="${pageContext.request.contextPath }/admin/toAdminEdit.do?adminId="+adminId;
            }
            function deleteAdmin(adminId,adminName){
            	//alert(adminId+adminName);
            	//window.location.href="${pageContext.request.contextPath }/admin/deleteAdmin.do?adminId="+adminId;
				layer.confirm("删除用户信息【"+adminName+"】, 是否继续",  {icon: 3, title:'提示'}, function(cindex){
            ...
            $("#chkAllAdmin").click(function(){
		       ...
		       $("#btnDeleteChkAdmin").click(function(){
		    	   //alert("--test--");
		    	   //1.获得选中的复选框的值
		    	   var chkModulars= $("input[name='adminId']:checked");
		    	  ...
		    	  if(chkModulars.length==0){
		    	   ...
		    	   layer.confirm("删除选择的用户信息, 是否继续",  {icon: 3, title:'提示'}, function(cindex){
	    				...
	    				$.ajax({
	    					type : "POST",
	    					url  : "${pageContext.request.contextPath }/admin/deleteAdmins.do",
	    					data :  params ,
	    					success : function(result) {
	    						if ( result.success ) {
	    							layer.msg("用户信息删除成功", {time:1000, icon:6}, function(){
